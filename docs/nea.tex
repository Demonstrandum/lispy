...
atomic (stream) =>
  condition if =>
    ...
    | otherwise =>
      ...
      \graph{parser-flow}{Parser outlined as a flow-chart.}
...
\section{Technical Solution \& Implementation}
  ...
  If you are unable to configure Python to work from your CMD, there is a third
  solution to running LISPY files. Any standard CPython installation from the
  official website will allow you to run any \code{.py} by simply double clicking
  the file in the Windows File Explorer.  The \code{windows\_execute.py} file
  provides a simple GUI interface implemented with tkinter which also comes with
  the standard Windows installation of CPython.  Hopefully this provides a
  useful interface for Windows users who might not be familiar or comfortable
  enough with the command line.
  ...
  \subsection{Parser}
    ...
    \subsubsection{Abstract Syntax Tree}
      ...
      \graph{tree_Hier}{Hierarchy of class inheritance in \code{/lispy/tree.py}.}
    ...
    \begin{itemize}
      ...
      this file implements:
        \begin{itemize}
          ...
          \item \code{take}, which returns the first n elements of a given list.
          \item \code{drop}, which returns a list missing the first n
                             elements a given list.
...
\section{Appraisal}
  ...
  \subsection{Comparison against other Languages}
   %% Write about this being a Lisp-1
   ...
   %% Write about Lisp-2 vs Lisp-1, namespace, higher order functions
   %%     CommonLisp v. Scheme
